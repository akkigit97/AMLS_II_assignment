...
# -------------------------------------
# Folder Paths (update these paths as needed)
# -------------------------------------
...
# -------------------------------------
# Dataset Class for Paired Bicubic Data (Track 1)
# -------------------------------------
class PairedBicubicDataset(Dataset):
    ...
    def __init__(self, lr_dir, hr_dir, lr_transform=None, hr_transform=None):
        ...
        # Use the minimum number of files so that LR and HR lists match
...
class Generator(nn.Module):
    """
    Generator network for SRGAN (Track 1).
    Upsamples LR images (e.g., 64x64) by a factor of 4 to produce HR images (e.g., 256x256).
    """
    def __init__(self, num_residual_blocks=4):
        ...
        # Upsample using two PixelShuffle layers for 4x scaling
...
class Discriminator(nn.Module):
    """
    Discriminator network for SRGAN (Track 1).
...
# -------------------------------------
# Training Function for Track 1 using GAN (SRGAN)
# -------------------------------------
def train_bicubic_sr_gan():
    print("Starting training for Track 1 (Bicubic)")
    
    # Define transforms for HR and LR images
    ...
    lr_transform = transforms.Compose([
        transforms.CenterCrop(64),  # assuming 4x downscaling (256/4 = 64)
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.5]*3, std=[0.5]*3)
    ])
    
    # Create training and validation datasets
    train_dataset = PairedBicubicDataset(TRAIN_LR_PATH, TRAIN_HR_PATH,
                                          lr_transform=lr_transform,
                                          hr_transform=hr_transform)
    valid_dataset = PairedBicubicDataset(VALID_LR_PATH, VALID_HR_PATH,
                                          lr_transform=lr_transform,
                                          hr_transform=hr_transform)
    ...
    # Tracking losses and validation PSNR
    generator_losses = []
    ...
    for epoch in range(num_epochs):
        ...
        # Validation: Compute PSNR on the validation set
        generator.eval()
        cumulative_psnr = 0.0
        with torch.no_grad():
    ...
    plt.tight_layout()
    plt.savefig("track1_srgan_performance.png")
    plt.show()
    
    # Save the trained generator model
    torch.save(generator.state_dict(), "track1_srgan_generator.pth")
    print("Training complete. Generator model saved as 'track1_srgan_generator.pth'.")

if __name__ == "__main__":
    train_bicubic_sr_gan()